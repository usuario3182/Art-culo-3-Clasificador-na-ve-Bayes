...
# Solo las clases con métricas distintas de cero
tabla1 <- data.frame(
  Clase = c("Photographs / Videos / EVP", "Shadow People"),
  Precision = c(0.0563, 0.0286),
  Recall = c(1, 0.0476),
  F1_Score = c(0.1066, 0.0357)
)

kable(tabla1, 
      caption = "Métricas por clase con valores distintos de cero obtenidas del clasificador naivebayes.")
...
#| echo: false
tabla2 <- data.frame(
  Métrica = c("Macro Precision", "Macro Recall", "Macro F1-Score"),
  Valor = c(0.0042, 0.0524, 0.0071)
)

kable(tabla2,
      caption = "Promedios de las métricas para todas las clases con el clasificador e1071 y naivebayes.")
...
#| echo: false
tabla3 <- data.frame(
  Categoria = c("Ghost Tours & Haunted Hotels", "Haunted Items", "Photographs / Videos / EVP", "Shadow People", "Succubus / Incubus / Sexual Ghosts"),
  Relatos = c(71, 46, 79, 63, 97)
)

kable(tabla3,
      caption = "Distribución de relatos por categoría (solo categorías con valores distintos de 100).")
...
#| echo: false
tabla4 <- data.frame(
  Actual = c("Haunted Places", "Other"),
  Predicho_HauntedPlaces = c(29, 528),
  Predicho_Other = c(0, 0)
)

kable(tabla4,
      caption = "Matriz de confusión para la clasificación dicotómica con case_when(): Haunted Places vs Other.")
...
# Crear un dataframe combinando los dos clasificadores
promedios <- data.frame(
  Clasificador = c("e1071", "naivebayes"),
  Macro_Precision = c(0.0042, 0.0042),
  Macro_Recall = c(0.0524, 0.0524),
  Macro_F1_Score = c(0.0071, 0.0071)
)

# Mostrar tabla
kable(promedios,
      caption = "Comparación de métricas promedio entre los clasificadores e1071 y naivebayes.",
      digits = 4)
...
promedios <- data.frame(
  Clasificador = c("e1071", "naivebayes"),
  Macro_Precision = c(0.026, 0.026),
  Macro_Recall = c(0.5, 0.5),
  Macro_F1_Score = c(0.0495, 0.0495)
)

# Mostrar tabla
kable(promedios,
      caption = "Comparación de métricas promedio para la clasificación dicotómica (case_when) entre los clasificadores e1071 y naivebayes.",
      digits = 4)
...
Por otro lado, los resultados del clasificador Naive Bayes con la distribución Poisson mostraron muy baja efectividad con métricas cercanas a cero para casi todas las clases, salvo un par con valores relativamente mejores pero aún insuficientes. Esto indica que el modelo no logró identificar correctamente las categorías de las historias en la mayoría de los casos, reflejando un desempeño global poco útil para la tarea de clasificación en este contexto.
