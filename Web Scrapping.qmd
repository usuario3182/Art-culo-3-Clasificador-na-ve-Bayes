...
```{r}
#saveRDS(head(df,2), "objetos/head_relatos_y_categorias.rds")
```
...
#saveRDS(tabla_fre_cat, "objetos/tabla_fre_cat.rds")
kable(tabla_fre_cat)
```
...
#saveRDS(diagrama, "objetos/diagrama_frecuencia_cat.rds")
diagrama
```
...
categorias = c("A Haunted Life", "The Haunted House")

# Top palabras por categoría
top_palabras = my_words %>%
  filter(category %in% categorias) %>%       # Filtra solo las dos categorías
  anti_join(stop_words, by = "word") %>%    # Elimina stop words
  count(category, word, sort = TRUE) %>%    # Cuenta frecuencia
  group_by(category) %>%
  slice_head(n = 10)                        # Top 10 palabras


diagrama = top_palabras %>%
  ggplot(aes(x = n, y = reorder_within(word, n, category), fill = category)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~category, scales = "free") +
  scale_y_reordered() +
  labs(
    x = "Frecuencia",
    y = "Palabra",
    title = "Top 10 palabras más frecuentes en A Haunted Life"
  )
...
```{r}
top_palabras = my_words_clean %>%
  count(category, word, sort = TRUE) %>%
  group_by(category) %>%
  slice_head(n = 5) %>%  # solo la palabra más frecuente por categoría
  ungroup()

# Gráfico estilo facetas, una fila por categoría
diagrama_top = top_palabras %>%
  ggplot(aes(x = n, y = reorder_within(word, n, category), fill = category)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~category, scales = "free") +
  scale_y_reordered() +
  labs(
    x = "Frecuencia",
    y = "Palabra",
    title = "Palabra más frecuente por categoría"
  )

saveRDS(diagrama_top, "objetos/diagrama_top.rds")
diagrama_top
...
saveRDS(diagrama_he_she, "objetos/diagrama_he_she.rds")
diagrama_he_she
```
